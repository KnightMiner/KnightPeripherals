...
public class TurtleBow implements ITurtleUpgrade {
	...
	public TurtleCommandResult useTool(ITurtleAccess turtle, TurtleSide side, TurtleVerb verb, int direction) {
		switch (verb) {
			case Attack:
				...
				arrow.setPosition(x + 0.5D - (xOffset / 2.0D), y + 0.5D - (yOffset / 2.0D),
				        z + 0.5D - (zOffset / 2.0D));
	...
	public IIcon getIcon(ITurtleAccess turtle, TurtleSide side) {
		// turtles rotate the icon 90 degrees, and the easiest way to rotate it
		// back is to rotate my own texture

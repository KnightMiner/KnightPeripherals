...
public class TurtleHammer implements ITurtleUpgrade {
	...
	public ItemStack getCraftingItem() {
		if (canCraft()) {
			...
			KnightPeripherals.logger.info("Recipe for smashing turtle disabled");
			return null;
		}
	}
	...
	public TurtleCommandResult useTool(ITurtleAccess turtle, TurtleSide side, TurtleVerb verb, int direction) {
		switch (verb) {
			case Attack:
				// grab a fake player to use for basic checks, along with as a
				// target for when mobs get mad
				...
				if (!world.isAirBlock(x, y, z) && !world.getBlock(x, y, z).getMaterial().isLiquid()) {
					...
					if (rewards != null && rewards.size() > 0) {
						...
						if (block.getMaterial().isToolNotRequired()) {
							// and place the normal block drops into the
							// inventory
					...
					// all cases leading here mean we dug the block, so remove
					// the block, play a sound, and return true
					world.setBlockToAir(x, y, z);

					if (!world.isRemote)
						world.playSoundEffect(x + 0.5D, y + 0.5D, z + 0.5D, block.stepSound.getBreakSound(), 1.0F,
						        0.8F);

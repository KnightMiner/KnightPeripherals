		...
		switch (verb) {
			...
			case Dig:
				...
				if (!world.isAirBlock(pos) && !world.getBlockState(pos).getBlock().getMaterial().isLiquid()) {
					...
					// HammerRegistry.getRewards(block, blockMeta);

					// if we have something
					if (HammerRegistry.isHammerable(state)
					        && (block.getMaterial().isToolNotRequired() || block.getHarvestLevel(state) <= 3)) {
